...
class OfflineDownloadRepositoryImpl(
    ...
    private fun loadChapterRecords(
        sourceId: String,
        mangaUrl: String,
    ): List<OfflineDownloadedChapterRecord> {
        val keyPrefix = offlineChapterDataPrefix(mangaUrl)
    ...
    private fun decodeChapterRecord(raw: String): OfflineDownloadedChapterRecord? {
        return runCatching {
            json.decodeFromString<OfflineDownloadedChapterRecord>(raw)
        }.getOrNull()
    }

    private fun chapterDataPrefix(mangaUrl: String): String {
        return offlineChapterDataPrefix(mangaUrl)
    }
    ...
    private fun autoDownloadKey(mangaUrl: String): String {
        return offlineAutoDownloadKey(mangaUrl)
    }

    private companion object {
        private const val AUTO_DOWNLOAD_ENABLED = OFFLINE_AUTO_DOWNLOAD_ENABLED
        private const val AUTO_DOWNLOAD_DISABLED = OFFLINE_AUTO_DOWNLOAD_DISABLED
    }
}
